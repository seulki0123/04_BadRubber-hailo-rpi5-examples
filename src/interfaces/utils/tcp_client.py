...
class TCPClient(ProcessLogger, Connection):
    def __init__(self, host, port, name, reconnect_interval=2.0):
    ...
    def send(self, data):
        ...
        # NOTE:
        # Message framing is currently JSON + newline.
        # If binary or large payloads are needed, extract framing into a codec.
    ...
    def _connect(self):
        try:
            ...
            self.log_error(f"{self.host}:{self.port} connect failed: {e}. Retry in {self.reconnect_interval} sec...")
            self.socket = None
